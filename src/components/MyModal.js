...
class MyModal extends Component {
  ...
  add(){
    this.props.addToList(this.state.countryDb, this.state.number);
    this.setState({country: "", countryDb: "", number: ""})
  }
  render() {
    return (
      <View style={styles.modal}>
        <View style={styles.header}>
            <TouchableOpacity style={styles.exit} onPress={()=> this.props.showModal()}>
        <AntDesign style={styles.icon}
              color="white"
              name="close"
              size={20}/>
            </TouchableOpacity>
          <Text style={styles.title}>Añadí las figuritas que tenés</Text>
