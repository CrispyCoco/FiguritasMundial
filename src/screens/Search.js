  ...
  componentDidMount() {
    db.collection("figus")
      ...
      .onSnapshot((docs) => {
        ...
        this.setState({ figus: figus.data.owned });
      });
  }
  find() {
    let tempList = [];
    this.setState({ have: false, figu: "", listita: [] }, () => {
      this.state.figus.forEach((figu) => {
        if (this.state.countryDb.lable == figu.lable) {
          if (this.state.number == figu.number) {
            this.setState({ have: true, figu: figu, searched: true });
            return;
  ...
  render() {
    return (
      <View style={{ flex: 1, width: "100%" }}>
        ...
        <View style={styles.searchBar}>
          <View style={styles.drops}>
            <SelectCountry
              ...
              onChange={(e) => {
                console.log(e);
                this.setState({ country: e.value, countryDb: e });
              }}
            />
            <Dropdown
              style={styles.dropdown}
        ...
        {this.state.searched ? (
          this.state.listita.length > 0 ? (
            <View style={styles.containerList}>
              <Text style={styles.newsList}>
                Tenés {this.state.listita.length} de{" "}
                {this.state.listita[0].lable}
              </Text>
              <ScrollView contentContainerStyle={styles.stickerList}>
                <FlatList
                  data={this.state.listita}
            ...
            <View style={styles.container}>
              <Text style={styles.news}>Ya la tenés!</Text>
              <Sticker figu={this.state.figu} />
            </View>
          ) : (
            <View style={styles.container}>
